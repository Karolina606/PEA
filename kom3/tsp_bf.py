...
def breadth_first(matrix):
    ...
    node_in_queue_flag = True
    each_node = node_queue.get()
    while node_in_queue_flag:
        for i in range(1, n):
            if i != each_node.number and each_node.nodes_in_trace[i] is False:
                if each_node.cost + matrix[each_node.number][i] < 2 * mst and each_node.cost + \
                        matrix[each_node.number][i] < mini:
                    new_bool_nodes_tab = each_node.nodes_in_trace.copy()
                    new_bool_nodes_tab[i] = True
                    new_node = node(each_node, i, new_bool_nodes_tab, each_node.cost + matrix[each_node.number][i])
...
def work(name, matrix):
    ...
    path, cost = breadth_first(matrix)
...
def main():
    ...
    try:
        ...
        result_out_file_bs = config['result']['tsp_bf']
