...
class node:
    ...
    def nodes_to_string(self):
        text = ''
        for i in range(len(self.nodes_in_trace)):
            if self.nodes_in_trace[i] is True:
                text += str(i) + ' '
        return text

    def __str__(self):
        return "Number: " + str(self.number) + ", Cost: " + str(self.cost) + ", Way: " + self.nodes_to_string()


def depth_first(matrix):
    global mini
    global mini_node
    global node_stack
    ...
    current_mst = mst_find(matrix)
    print("mst dla: " + str(n) + "mst: " + str(current_mst))

    # inicjowanie pierwszego węzła
    bool_nodes_tab[0] = True
    first_node = node(None, 0, bool_nodes_tab, 0)
    node_stack.append(first_node)

    visit_neig(first_node)
...
def visit_neig(this_node: node):
    ...
    if False not in this_node.nodes_in_trace:
        return 0
    else:
        for i in range(1, len(current_matrix)):
            new_node = None
            if i != this_node.number and this_node.nodes_in_trace[i] is False:
                if this_node.cost + current_matrix[this_node.number][i] < 2 * current_mst and this_node.cost + \
                        current_matrix[this_node.number][i] < mini:
                    new_bool_nodes_tab = this_node.nodes_in_trace.copy()
                    new_bool_nodes_tab[i] = True
                    new_node = node(this_node, i, new_bool_nodes_tab, this_node.cost + current_matrix[this_node.number][i])

                    if False in new_bool_nodes_tab:
                        node_stack.append(new_node)

                    if False not in new_node.nodes_in_trace:
                        if new_node.cost + current_matrix[new_node.number][0] < mini:
                            mini = new_node.cost + current_matrix[new_node.number][0]
                            mini_node = new_node
                            print(mini)
                            if (mini - current_mst)/current_mst < 0.14:
                                end = True
                                break
                    if not end:
                        visit_neig(new_node)
...
def main():
    ...
    try:
        ...
        result_out_file_bs = config['result']['tsp_df']
